...
def main():
    ...
    run_pipeline_counter(input_file)
    files = os.listdir('.')
    filename = ''
    for file in files:
        if 'number' in file:
...
def run_pipeline_asc(input_file, output_file, shape):
    with beam.Pipeline(options = PipelineOptions()) as p:
        words = read_data_and_prepare(p,input_file) 
        ascending_sort = words | beam.combiners.Top.Of(shape,key = lambda x: x[1])
...
def run_pipeline_desc(input_file, output_file, shape):
    with beam.Pipeline(options = PipelineOptions()) as p:
        words = read_data_and_prepare(p,input_file) 
        descending_sort = words | beam.combiners.Top.Of(shape,key = lambda x: x[1], reverse = True)
        descending_sort | beam.io.WriteToText(f'{output_file}descend', file_name_suffix='.txt')

def run_pipeline_counter(input_file):
    with beam.Pipeline(options=PipelineOptions()) as pipeline2:
        words =  read_data_and_prepare(pipeline2,input_file)
        total_tuples = words | beam.combiners.Count.Globally() 
        total_tuples | beam.io.WriteToText('number', file_name_suffix='.txt')

def read_data_and_prepare(pipeline,input_file):
